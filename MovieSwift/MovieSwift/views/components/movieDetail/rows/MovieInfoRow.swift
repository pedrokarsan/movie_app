        ...
        HStack {
            if let date = movie.release_date {
                Text(date.prefix(4)).font(.subheadline)
                    .accessibilityIdentifier("movie_release_year")
            }
            if let runtime = movie.runtime {
                Text("• \(runtime) minutes")
                    ...
                    .accessibilityIdentifier("movie_duration")
            }
            if let status = movie.status {
                Text("• \(status)")
                    ...
                    .animation(asyncTextAnimation)
                    .transition(asyncTextTransition)
                    .accessibilityIdentifier("movie_status")
            }
        }
        ...
        Group {
            if movie.production_countries?.isEmpty == false {
                Text("\(movie.production_countries!.first!.name)")
                    .font(.subheadline)
                    .foregroundColor(.white)
                    .accessibilityIdentifier("production_country")
            }
        }
    }
