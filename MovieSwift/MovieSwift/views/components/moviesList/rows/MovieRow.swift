...
struct MovieRow: ConnectedView {
    ...
    func body(props: Props) -> some View {
        HStack {
            ...
            .fixedSize()
            .accessibilityIdentifier("movie_poster")
            .animation(.spring())
            VStack(alignment: .leading, spacing: 8) {
                Text(props.movie.userTitle)
                    ...
                    .lineLimit(2)
                    .accessibilityIdentifier("movie_title")
                HStack {
                    ...
                    Text(formatter.string(from: props.movie.releaseDate ?? Date()))
                        ...
                        .lineLimit(1)
                        .accessibilityIdentifier("movie_release_date")
                }
                Text(props.movie.overview)
                    .foregroundColor(.secondary)
                    .lineLimit(3)
                    .truncationMode(.tail)
                    .accessibilityIdentifier("movie_overview")
            }.padding(.leading, 8)
        }
        .padding(.top, 8)
        .padding(.bottom, 8)
